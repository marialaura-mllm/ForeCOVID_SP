...
for (w in 1:76){
  ...
  # Selecting only women and excluding 2020/21/22
  E <- as.matrix(E_start[19:36,w:(length(E_start)-3)])
  LMX <- as.matrix(log(LMX_start[19:36,w:(length(LMX_start)-3)]))
  Y <- as.matrix(Y_start[19:36,w:(length(Y_start)-3)])
  ...
  for(s in 1:nS){
    ...
    e0 <- apply(exp(nmx),2,e0.mx,x=a,sex="F")
    
    SIMk <- rbind(SIMk, e0) # Saving the e0 (2020-2050)
    SIMnmx <- rbind(SIMnmx, kappa.sim) # Saving the kappas (2020-2050)
  }
...
for (w in 1:76){
  ...
  # Selecting only women , data from 1920 to 2022
  E <- as.matrix(E_start[19:36,w:length(E_start)])
  LMX <- as.matrix(log(LMX_start[19:36,w:length(LMX_start)]))
  Y <- as.matrix(Y_start[19:36,w:length(Y_start)])
...
for (w in 1:76){
  ...
  # Selecting only women, data from 1920 to 2023
  E <- as.matrix(E_start[19:36,w:length(E_start)])
  LMX <- as.matrix(log(LMX_start[19:36,w:length(LMX_start)]))
  Y <- as.matrix(Y_start[19:36,w:length(Y_start)])
...
for (w in 1:76){
  ...
  aux_nmx <- exp(c(rep(1,18), lc_LMX_2023[((w*18)-17):(w*18),]))
...
for (w in 1:76){
  ...
  # Selecting only women, data from 1920 to 2024
  E <- as.matrix(E_start[19:36,w:length(E_start)])
  LMX <- as.matrix(log(LMX_start[19:36,w:length(LMX_start)]))
  Y <- as.matrix(Y_start[19:36,w:length(Y_start)])
...
for (w in 1:76){
  ...
  # Taking the estimates from s1 per iteration
  aux_nmx <- exp(c(rep(1,18), lc_LMX_2024[((w*18)-17):(w*18),]))
  aux_deaths <- aux_pop*aux_nmx
...
for (w in 31:76){
  ...
  e0.obs.real <- apply(exp(LMX),2,e0.mx,x=a,sex="F")
  
  # Adjusting kt for the e0
  koptim <- function(par,Alpha,Beta,e0.obs,x,sex){
    Kappa <- par[1]
    lmx.lc <- Alpha+Beta*Kappa
    e0.est <- e0.mx(mx=exp(lmx.lc),x=a,sex="F") #e0 adjustment
  ...
  for (i in 1:n1){
    KappaSecStep <- optimize(f=koptim, interval=c(-150,150), Alpha=Alpha, 
                             Beta=Beta, e0.obs=e0.obs.real[i], x=a, sex="F")
    Kappa[i] <- KappaSecStep$minimum
  }
...
for (w in 31:76){
  ...
  # Taking the estimates from s1 per iteration
  aux_nmx <- exp(c(rep(1,18), lm_LMX_2023[(((w-30)*18)-17):((w-30)*18),]))
  aux_deaths <- aux_pop*aux_nmx
...
for (w in 31:76){
  ...
  # Saving kappas 1 column per forecasted year, one row per sim and starting year
  lm_s5 <- rbind(lm_s5, cbind(NA, NA, SIMk)) # NA for the years 2023/2024
...
for (w in 31:76){
  ...
  # Taking the estimates from s1 per iteration
  aux_nmx <- exp(c(rep(1,18), lm_LMX_2024[(((w-30)*18)-17):((w-30)*18),]))
  aux_deaths <- aux_pop*aux_nmx
  ...
  # Saving kappas 1 column per forecasted year, one row per sim and starting year
  lm_s6 <- rbind(lm_s6, cbind(NA, NA, SIMk)) # NA for the year 2023/2024
...
write.table(aux2, 'summary_female.txt')

# Getting estimates for 1920-2022
LMX_start <- read_xlsx('nmx.xlsx') # Death rates
LMX <- as.matrix(log(LMX_start[19:36,3:length(LMX_start)]))

aux_obs <- data.frame(year = seq(1920, 2022),
                      e0.med = apply(exp(LMX),2,e0.mx,x=a,sex="F"))
  

reg_f <- lm(e0_Fem~Year, data=aux_bp_all) # Obtain Oeppen-Vaupel curve for Female
...
ggsave(path = "plots/",filename = "figure_fore_female.pdf", width = 9, height = 5)
ggsave(path = "plots/",filename = "figure_fore_female.png", width = 9, height = 5)
